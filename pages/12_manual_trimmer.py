...
if st.session_state.image_queue:
    ...
    for img_path in list(st.session_state.image_queue):
        ...
        col_left, col_center, col_right, col_action = st.columns([1, 4, 1, 1], vertical_alignment="center")
    ...
    if st.button("🚀 Apply Crops & Save", type="primary"):
        for img_path in st.session_state.image_queue:
            ...
            # Skip file operation if no crop is requested
        ...
        st.success("Cropping applied to queue. Originals overwritten.")
        st.session_state.image_queue = [] 
        st.rerun()
elif folder_path:
