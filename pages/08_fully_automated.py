...
def cleanup_page_seams(wikitext):
    ...
    # 2. Fix Sentence Flow (Negative Lookahead for tables/headers)
    wikitext = re.sub(
        r'(\{\{page\|[^}]+\}\})\n(?![{|!=*#])',
        r'\1',
        wikitext
...
st.info(f"📂 Found {total_files} PDF files.")
...
with col1:
    start_btn = st.button("🚀 Start / Resume", type="primary", use_container_width=True)
with col2:
...
if start_btn:
    ...
    for i in range(current_idx, end_idx):
        ...
        status_container.caption(f"Target: `{wiki_title}`")
        
        start_page = state['current_page_num'] if i == state['current_file_index'] else 1
        page_num = start_page
        ...
        while True:
            ...
            try:
                ...
                # 3. Header Injection (Page 1 Only)
                if page_num == 1:
                ...
                # 4. NOTOC Injection (Last Page Only)
                doc = fitz.open(pdf_path)
                ...
                if inject_error:
                    st.error(f"CRITICAL ERROR on Page {page_num}: {inject_error}")
                    st.stop()

                # 7. Upload Current Page
                ...
                # 8. FINAL CLEANUP (Last Page Only)
                #    Happens AFTER the final page content is uploaded
                if is_last_page:
                    log_area.text(f"🧹 Running final seam cleanup on {short_name}...")
                    
                    # A. Fetch full document again (now contains all new pages)
                ...
                # 9. Update State (Move to next page)
    ...
    # Save State to resume NEXT file
    save_state(end_idx, 1, "done")
