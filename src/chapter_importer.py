...
def link_chapter_to_author(wbi, chapter_item_id, author_item_id):
    ...
    new_claim = Item(value=chapter_item_id, prop_nr='P11') # has authored
...
def link_chapter_to_book(wbi, chapter_item_id, book_item_id):
    ...
    new_claim = Item(value=chapter_item_id, prop_nr='P59')  # P59 = has chapter
...
def create_chapter_item(wbi, title, page_range, author_item_ids, book_item_id):
    ...
    # P12: Instance of Written Work (Q4581)
...
def import_chapters_to_wikibase(book_qid, chapters_data):
    """
    Orchestrates the creation of chapters and linking them to the parent book.
    ...
    for chapter in chapters_data:
        ...
        for author_name in authors:
            # Reusing the robust logic from your other importer
            aid = check_or_create_person(wbi, author_name, "author")
            author_ids.append(aid)
            if aid and not wbi_helpers.search_entities(author_name): 
                # If it was just created (rough check), log it
                logs.append(f"Created new author: {author_name} ({aid})")

        # 2. Create Chapter
        chapter_qid = create_chapter_item(wbi, title, page_range, author_ids, book_qid)
        logs.append(f"Created chapter: '{title}' ({chapter_qid})")

        # 3. Link Author -> Chapter
        for aid in author_ids:
            link_chapter_to_author(wbi, chapter_qid, aid)
